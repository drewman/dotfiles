...
#+begin_src emacs-lisp
  ...
  (defun doom/rss-next ()
    ...
    (with-current-buffer (elfeed-search-buffer)
      (forward-line)
      (call-interactively 'doom/rss-open)))

  (defun doom/rss-previous ()
    ...
    (let ((search-buffers (doom-buffers-in-mode 'elfeed-search-mode))
      ...
      (dolist (b search-buffers)
        (with-current-buffer b
          (remove-hook 'kill-buffer-hook #'doom/rss-cleanup-h :local)
          (kill-buffer b)))
  ...
  (defun doom/rss-dead-feeds (&optional years)
  ...
  (defun doom/rss-put-sliced-image-fn (spec alt &optional flags)
    "TODO"
  ...
  (defun doom/rss-render-image-tag-without-underline-fn (dom &optional url)
    "TODO"
    (let ((start (point)))
      (shr-tag-img dom url)
...
#+begin_src emacs-lisp
  (use-package elfeed
    ...
    (setq elfeed-search-filter "@2-week-ago "
          ...
          elfeed-show-entry-delete #'doom/rss-delete-pane
          shr-max-image-proportion 0.8)

    (make-directory elfeed-db-directory t)

    ;; Enhance readability of a post
    (add-hook 'elfeed-show-mode-hook #'doom/rss-elfeed-wrap-h)
    (add-hook 'elfeed-search-mode-hook
      (add-hook 'kill-buffer-hook #'doom/rss-cleanup-h nil 'local))
    ...
    (add-hook 'elfeed-show-mode-hook
              (setq shr-put-image-function 'doom/rss-put-sliced-image-fn
                    shr-external-rendering-functions '((img . doom/rss-render-image-tag-without-underline-fn)))))
  ...
  (use-package elfeed-dashboard
    :after elfeed
    :load-path (lambda () (expand-file-name "lisp/elfeed-dashboard" user-emacs-directory))
    :config
    (progn
